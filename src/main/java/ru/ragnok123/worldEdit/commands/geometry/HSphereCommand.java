...
public class HSphereCommand extends WECommand {
	...
	public void execute(Player p, WEPlayer dat, String[] args) {
		if (args.length != 2) {
			p.sendMessage(WorldEdit.getPrefix() + TextFormat.RED + "Use //hsphere <block> <radius>");
			return;
		}
		...
		p.sendMessage(TextFormat.AQUA + String.valueOf(WorldUtils.hsphere(dat, p.getPosition(), Integer.valueOf(args[1]), b)) + TextFormat.GREEN + " block(s) have been changed.");
	}
